...
def simulate_key_combo(mod_char: tuple[str, str]) -> bool:
    """Press mod+char (e.g. ctrl+c). Uses ydotool on Wayland, pynput elsewhere."""
    mod, char = mod_char
    ...
    if sys.platform.startswith("linux") and is_wayland():
        ...
        log("[linux_input] ydotool key failed — checking /dev/uinput access...")
        if not os.access("/dev/uinput", os.W_OK):
            log("[linux_input] /dev/uinput not writable. User needs input group + re-login.")
        return False
