"""Ensure only one Prompt Enhancer process runs at a time."""
...
def _stop_hint() -> str:
    if sys.platform == "win32":
        return "Close the other Prompt Enhancer from the system tray, or end it in Task Manager."
    if sys.platform == "darwin":
        return (
            "Quit the other copy from the tray, or: launchctl unload "
            "~/Library/LaunchAgents/com.promptenhancer.app.plist"
        )
