...
from prompt_enhancer.core.prompts import MODE_LABELS
...
def fetch_ollama_models(base_url: str, timeout: float = 5.0) -> list[str]:
    """Fetch the list of installed models from an Ollama server.

    Ollama exposes GET /api/tags which returns:
        {"models": [{"name": "llama3.1:8b", ...}, ...]}

    Works for any network-reachable Ollama server — not just localhost.
    The user may enter the URL with or without a trailing /v1 (OpenAI-compat
    suffix); we strip it and hit the native /api/tags endpoint.
    """
    url = base_url.strip().rstrip("/")
    # Strip /v1 if present — /api/tags is at the server root, not under /v1
    if url.endswith("/v1"):
        url = url[:-3]
    tags_url = url + "/api/tags"
...
def validate_custom_prompt(prompt: str) -> str | None:
    """Validate a custom system prompt. Kept for test_connection compatibility."""
    if "\x00" in prompt:
...
def _valid_mode_ids(config: dict) -> set[str]:
    """Valid mode IDs — just the built-in modes now (custom modes removed)."""
    return set(MODE_LABELS.keys())
...
def _normalize_config(config: dict) -> dict:
    ...
    merged["ollama_url"] = str(merged.get("ollama_url", OLLAMA_DEFAULT_URL)).strip() or OLLAMA_DEFAULT_URL
...
def _migrate_config(config: dict) -> dict:
    ...
    if config.get("config_version", 0) < CONFIG_VERSION:
        ...
        changed = True
    # Migrate old 30s timeout to 90s — 30s was too short for NVIDIA's free tier.
    if config.get("timeout", 90) == 30:
