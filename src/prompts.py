...
_PROMPTIFY_RULES = (
    ...
    "the user's draft into a high-quality AI-ready prompt.\n"
    "STRICT RULES:\n"
    ...
    "10. Preserve the original scope unless the user strongly implies additional context.\n"
    "11. Return only the enhanced prompt.\n"
    "\n"
    "INTENT DETECTION:\n"
...
SYSTEM_PROMPTS: dict[str, str] = {
    ...
    "professional": (
        _REWRITER_RULES
        + "\n\nTask:\n"
        "Rewrite the text using professional business language, proper grammar, "
        "clear structure, and a formal tone."
    ),

    "shorten": (
        _REWRITER_RULES
        + "\n\nTask:\n"
        "Reduce the length significantly while preserving every important idea, "
        "request, question, and intent."
    ),

    "expand": (
        _REWRITER_RULES
        + "\n\nTask:\n"
        "Improve clarity by expanding existing ideas, refining wording, and "
        "improving structure. Do not introduce new topics."
    ),

    "casual": (
        _REWRITER_RULES
        + "\n\nTask:\n"
        "Rewrite using a natural, friendly, and conversational tone while "
        "preserving intent."
    ),
