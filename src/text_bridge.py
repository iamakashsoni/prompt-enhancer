...
the result. Works on Windows, macOS, and Linux (X11 with xclip/xsel;
Wayland with wl-clipboard PRIMARY + ydotool).
"""
import os
import subprocess
import sys
...
if sys.platform.startswith("linux"):
    from linux_input import is_wayland, simulate_key_combo
else:
    def is_wayland() -> bool:
        return False

    def simulate_key_combo(mod_char):  # type: ignore[no-redef]
        from pynput.keyboard import Controller, Key
        mod, char = mod_char
        key = Key.cmd if mod == "cmd" else Key.ctrl
        kb = Controller()
        with kb.pressed(key):
            kb.press(char)
            kb.release(char)
...
if not (sys.platform.startswith("linux") and is_wayland()):
    from pynput.keyboard import Controller, Key
...
def capture_selected_text() -> tuple[str, str] | None:
    ...
    """
    original = _clip_read()
    wayland = sys.platform.startswith("linux") and is_wayland()
