...
from prompts import MODE_LABELS, STRICT_RETRY_SUFFIX, SYSTEM_PROMPTS  # noqa: F401
...
_STRUCTURED_MODES = frozenset({"promptify", "expand"})
...
def _length_budget(original: str, mode: str) -> int:
    n = len(original)
    if mode == "shorten":
        return max(int(n * 1.1), n + 80)
    if mode == "promptify":
        return max(int(n * 2.5), n + 500)
    if mode == "expand":
        return max(int(n * 2.0), n + 350)
    if n >= 400:
...
def _max_output_tokens(text: str, mode: str) -> int:
    ...
    if mode == "shorten":
        ...
        cap = min(max(int(base * 1.8), 512), 2048)
    else:
...
def _resolve_system_prompt(mode: str, config: dict) -> str:
    ...
    custom_modes: dict = config.get("custom_modes", {})
    if mode in custom_modes:
        prompt = custom_modes[mode].get("system_prompt", "").strip()
        if not prompt:
            name = custom_modes[mode].get("name", mode)
            raise EnhancementError(
                f"Custom mode '{name}' has no system prompt. "
                "Open Settings → Custom Modes to add one."
            )
        return prompt

    raise EnhancementError(
        f"Unknown enhancement mode: {mode!r}. "
        "It may have been deleted — open Settings to check."
    )
...
def _resolve_model(mode: str, config: dict) -> str:
    ...
    Per-mode overrides let users trade quality for speed on a per-mode basis.
    For example, "casual" and "shorten" work fine on a faster 8B model
    (~3-5x lower latency than 70B), while "enhance" and "promptify" benefit
    from the 70B default. The override is purely opt-in via config.json —
    no UI surface, no behaviour change for existing users.
    """
...
def all_mode_ids(config: dict) -> list[str]:
    return list(MODE_LABELS.keys()) + list(config.get("custom_modes", {}).keys())
...
def mode_display_name(mode_id: str, config: dict) -> str:
    ...
    if mode_id in custom_modes:
        return custom_modes[mode_id].get("name", mode_id)
    return mode_id
