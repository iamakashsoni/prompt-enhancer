...
os=$(detect_os)
case "$os" in
    wsl)     die "WSL detected. Run from native Windows with: install.bat" ;;
...
if $VENV_PY src/autostart.py enable 2>/dev/null; then
    ok "Autostart enabled"
else
    warn "Autostart setup failed — the app will run but won't start on login.
Enable manually: $VENV_PY src/autostart.py enable"
fi
